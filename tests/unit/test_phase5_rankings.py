...
def test_rankings_endpoint_returns_ranked_items_with_positions(monkeypatch: Any) -> None:
    recommendations = [
        {
            "item_id": 11840,
            "horizon_hours": 1,
    ...
    response = client.get("/api/v1/rankings?horizon_hours=1&limit=2")
    assert response.status_code == 200
    payload = response.json()
    assert len(payload) == 2
    assert payload[0]["item_id"] == 4151
    assert payload[0]["rank"] == 1
    assert payload[1]["item_id"] == 11840
...
def test_rankings_endpoint_applies_filters_and_returns_champion_model_fields(monkeypatch: Any) -> None:
    ...
    response = client.get(
        "/api/v1/rankings?horizon_hours=1&signal_label=stable&liquidity_status=healthy&drift_state=stable&top_n=1"
    )
    assert response.status_code == 200
    payload = response.json()
    ...
    assert payload[0]["item_id"] == 4151
    assert payload[0]["rank"] == 1
    assert payload[0]["champion_model_name"] == "naive_last"
    assert payload[0]["champion_model_version"] == "1.0.0"
